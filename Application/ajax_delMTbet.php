...
$arr = array();
$type = 6;
$cheterm = get_query_val('fn_mtorder', 'term', array('id' => $delid));
$chemoney = get_query_val('fn_mtorder', 'money', array('id' => $delid));
...
if($chedan){
    ...
    echo json_encode($arr);
    exit();
}elseif(get_query_val("fn_mtorder", "status", array("id" => $delid)) == '已撤单'){
    $arr['success'] = false;
    $arr['msg'] = "您选择的订单已经撤销,相关金额已经退回您的余额,请点击左侧菜单进行刷新!";
    echo json_encode($arr);
    exit();
}else{
    update_query("fn_mtorder", array("status" => "已撤单"), array("id" => $delid));
